    ...
    :data-turn="richPositionData.turn">
        ...
        <g v-if=isInitial>
            ...
            <g v-for="(token, i) in richPositionData.tokens" 
                ...
                <rect
                    :x="centers[token.to][0] - 7"
                    :y="centers[token.to][1] - 7" 
                    :width="14"
                    :height="14"
                    :rx="2"
                    :ry="2"
        ...
        <g v-else>
            ...
            <g v-for="(token, i) in richPositionData.tokens" 
                ...
                <g v-if="token.token != '-'">
                    <rect
                        :x="centers[token.to][0] - 2"
                        :y="centers[token.to][1] - 2" 
                        :width="4"
                        :height="4"
                        :rx="0.2"
                        :ry="0.2"
...
<script lang="ts" setup>
    ...
    function calcTranslateAndScale(i: number) {
        if (i < 16) { 
    ...
    const centers = computed(() => {
            ...
            for (const i of Array(300).keys()) {
                if (i < 16) { 
                    ...
                    const j = Math.floor((i - 16) / 16);
                    const k = i - 16 * (j + 1);
                    lst.push([25 * (j % 4) + 3.125 + 6.25 * (k % 4), 25 * Math.floor(j / 4) + 3.125 + 6.25 * Math.floor(k / 4), 0.20]);
                } else {
                    const k = i - 272;
