    ...
    <div id="app-game-vvh-body-winbyview">
        ...
        <svg id="chart" :viewBox="`0 0 ${chartWidth} ${chartHeight}`" xmlns="http://www.w3.org/2000/svg">
            <template v-if="showVvhGuides">
                ...
                <text class="right-player-winning-direction" :x="gridRight" :y="chartHeight - winningDirectionHeight / 2"
                    dominant-baseline="middle" text-anchor="end">
                    <tspan id="player-2">{{ currentRightPlayerName }}</tspan>
                    Winning ⮕|
                </text>
            ...
            <!-- 0 Labels -->
            <text class="draw-symbol" :x="chartWidth / 2" :y="gridTop - xCoordinateHeight / 2" dominant-baseline="middle"
                text-anchor="middle">0</text>
            <text class="draw-symbol" :x="chartWidth / 2" :y="gridBottom + xCoordinateHeight / 2" dominant-baseline="middle"
                text-anchor="middle">0</text>
            ...
            <template v-if="currentValuedRoundId >= 2">
                <template v-for="roundNumber in currentValuedRoundId - 1" :key="roundNumber">
                    <line :class="`${currentValuedRounds[roundNumber].moveValue} link`"
                        :x1="winByNodeGridXPosition(roundNumber)" :y1="gridTop + roundNumber * rowHeight - rowHeight / 2"
