        ...
        <div id="app-game-body">
            <div id="app-game-body-main">
                ...
                <div id="app-game-body-main-function">
                    <button id="app-game-undo" @click="undid" :disabled="isInvalidUndo">Undo</button>
                    <button id="app-game-restart" @click="restarted">Restart</button>
                    <button id="app-game-redo" @click="redid" :disabled="isInvalidRedo">Redo</button>
                </div>
                <div id="app-game-body-main-stats">
                    <div id="app-game-round">Move #{{ game.getRound().getRoundNumber() }}</div>
                    <div v-if="options.getHintVisibility()" id="app-game-remoteness">Remoteness {{ game.getRound().getRemoteness() }}</div>
                </div>
...
<script lang="ts">
    ...
    export default class AppGame extends Vue {
        ...
        metaInfo(): { title: string } {
            return {
                title: this.game.getName() + " Game: " + this.game.getCurrentVariantData().description,
            };
        }
        initInterval(): void {
        ...
        get isInvalidRedo(): boolean {
            return this.game.getHistory().getCurrentRoundNumber() >= this.game.getHistory().getRoundArray().length;
        }
        redid(): void {
